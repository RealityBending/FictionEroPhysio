...
// Beep sequence (10 beeps) // change after
const TAP_beep_sequence = {
    timeline: [TAP_beep],
    repetitions: 10, // 10 beeps total
    on_start: function () {
        document.body.style.backgroundColor = "#FFFFFF";
        document.body.style.cursor = "auto";
    }
...
function create_TAP_trial(screen = "tap", trial_duration = null, markerColor = "white") {
    return {
        type: jsPsychHtmlKeyboardResponse,
        on_load: function () {
            // Assuming marker1 and marker2 are valid global references
            create_marker(marker1, markerColor); // Show color marker
            create_marker_2(marker2);            // Show position marker
        },
        stimulus: "<b>Keep tapping at the same rhythm!</b>", // Displayed instruction
...
// 4. Function to create a tapping phase (sequence of trials)
function create_TAP_sequence(screen = "TAP1", repetitions = 90, interval = 1429) {
    return {
        timeline: [
            create_TAP_trial(screen + "_waiting", null, "white"), // Untimed trial (starts with keypress)
            create_TAP_trial(screen + "_tap", interval, "black")  // Timed tapping trials
        ],
